...
class metroSys(object):
    ...
    def dsp(self):
        # Setting
        start, destination, sourceList, destinationList = (
            self.start,
            self.destination,
            self.sourceList,
    ...
    def output_path(self, change):
        if self.start == self.destination:
            print(
                "Cannot find a path between <{0}> and <{1}>".format(
                    valueTokeys[self.start][0], valueTokeys[self.destination][0]
                )
            )
            return None
        ...
        print(
            "The {0} Path from <{1}> to <{2}>:".format(
                userChosen, valueTokeys[self.start][0], valueTokeys[self.destination][0]
            )
        )
        ...
        print(
            "The Result is Depended On <{0} Path>\nTotal stations: {1}\nChange Train: {2} Times\nTotal time: {3}".format(
                userChosen, len(self.path), change, self.total_cost
            )
        )
