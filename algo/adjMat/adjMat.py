...
def main():
    ...
    DFS = DepthFisrtSearch(adjMat)
    DFS.dfs()
    DFS.result()
    BFS = BreadthFirstSearch(adjMat)
    BFS.bfs()
    BFS.result()
...
def read_file(file_path):
    try:
        return np.loadtxt(
            file_path, delimiter=",", ndmin=2, dtype=np.dtype([("A", "i4"), ("B", "i4"), ("Weight", "f4")])
        )[0]
    except:
        return np.loadtxt(
            file_path, delimiter=",", ndmin=2, dtype=np.dtype([("A", "S1"), ("B", "S1"), ("Weight", "f4")])
        )[0]
