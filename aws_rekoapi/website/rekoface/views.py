...
def retreiveData(data):
	...
	Age = 'The detected face is between ' + str(faceDetails['AgeRange']['Low']) + ' and ' + str(faceDetails['AgeRange']['High']) + ' years old'
	Gender = faceDetails['Gender']['Value']
	Emotions = [faceDetails['Emotions'][i]['Type'] for i in range(len(faceDetails['Emotions'])) if faceDetails['Emotions'][i]['Confidence'] >= 70]
	...
	Brightness = 'You seems like in a bright room' if faceDetails['Quality']['Brightness'] > 0.5 else 'You seems like in a room a bit be sombre'
